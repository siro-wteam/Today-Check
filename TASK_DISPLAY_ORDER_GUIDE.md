...
### 2.2 “오늘” 날짜 내 정렬 (오늘만 해당)

`groupTasksByDate` 안에서 **오늘**(`todayStr`) 버킷에 들어간 리스트만 별도 정렬합니다.
...
**오늘이 아닌 날짜**는 API에서 온 순서(due_date → due_time → created_at)를 그대로 유지하고, 별도 sort는 하지 않습니다.
...
|------------|---------------------------|------------------------|----------------------------------------|
| 주간 뷰    | Store (getAllTasksInRange) | groupTasksByDate       | 오늘만: original_due_date → due_time → created_at / 그외: API 순서 |
| 일별 뷰    | 동일                      | 동일                   | 동일                                   |
